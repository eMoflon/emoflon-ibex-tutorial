...
public void createAdministration() {
	...
	api.staff().apply();
	api.staff().apply();
	api.earlyshift().apply();
	api.lateshift().apply();
	api.nightshift().apply();
	api.patient().apply();
	api.patient().apply();
	api.patient().apply();
	api.patient().apply();
...
public void validateAdministration() {
	...
	if (api.findAdministration().countMatches() == 1) {
		System.out.println("The administration was created");
	} else
		System.out.println("Error, the administration was not created");
	
	if (api.findEarlyShift().countMatches() + api.findLateShift().countMatches() + api.findNightShift().countMatches() >= 3) {
