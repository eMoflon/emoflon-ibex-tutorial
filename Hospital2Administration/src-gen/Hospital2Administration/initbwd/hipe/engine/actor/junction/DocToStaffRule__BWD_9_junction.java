...
public class DocToStaffRule__BWD_9_junction extends GenericJunctionActor{
	private Map<Object, Collection<HMatch>> department_3_AttrMap = HiPEMultiUtil.createMap();
	private Map<Object, Collection<HMatch>> staff_3_AttrMap = HiPEMultiUtil.createMap();
	
	@Override
	protected void initializePorts(Map<String, ActorRef> name2actor, JunctionNode node) {
		constraints.add(this::check_constraint_5);
	...
	protected void registerMatchForAttributeChanges(HMatch match) {
		...
		Collection<HMatch> staff_6_Matches = staff_3_AttrMap.get(matchObjects[6]);
		if(staff_6_Matches == null) {
			staff_6_Matches = new LinkedList<>();
			staff_3_AttrMap.put(matchObjects[6], staff_6_Matches);
		}
	...
	protected void deregisterMatchForAttributeChanges(Set<HMatch> matches, HMatch match) {
		...
		Collection<HMatch> matches_1 = staff_3_AttrMap.get(matchObjects[6]);
		if(matches_1 != null) {
			matches.remove(match);
		}
	}
	...
	protected void changeAttribute(AttributeChanged<HMatch> message) {
		...
		if(obj instanceof AdministrationExample.Staff) {
			if(staff_3_AttrMap.containsKey(obj)) {
				for(HMatch attr_match : staff_3_AttrMap.get(obj)) {
					for(int i=0; i<ports.size(); i++) {
						Port<HMatch> port = ports.get(i);
						HMatch match = attr_match;
	...
	public boolean check_constraint_5(HMatch match, int index) {
		...
		csp_1.getVariables().add(new org.emoflon.ibex.tgg.operational.csp.RuntimeTGGAttributeConstraintVariable(true, d.getDID(), "int"));
		csp_1.getVariables().add(new org.emoflon.ibex.tgg.operational.csp.RuntimeTGGAttributeConstraintVariable(true, s2.getDInt(), "int"));
		csp_1.solve();
