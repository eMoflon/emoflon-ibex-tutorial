...
public class Staff_object_SP1 extends GenericObjectActor<AdministrationExample.Staff> {
	...
	protected void initializePorts(Map<String, ActorRef> name2actor, ObjectNode node) {
		...
		ports.add(new PortNodeRight<AdministrationExample.Staff>(node.getPorts().getPort().get(2), getSelf(), name2actor.get("Administration_staff_1_reference"), this::check_constraint_11 , 0   ));
		ports.add(new PortNodeMatch<AdministrationExample.Staff>(node.getPorts().getPort().get(1), getSelf(), name2actor.get("DoctorToPatient__BWD_41_localSearch"), this::returnTrue   , node.getName() ));
		ports.add(new PortNodeLeft<AdministrationExample.Staff>(node.getPorts().getPort().get(0), getSelf(), name2actor.get("Staff_shiftPlan_0_reference"), this::check_constraint_3 , 1   ));
	}
	
	public boolean check_constraint_11(AdministrationExample.Staff s2, int index) {
	...
	public boolean check_constraint_3(AdministrationExample.Staff s2, int index) {
