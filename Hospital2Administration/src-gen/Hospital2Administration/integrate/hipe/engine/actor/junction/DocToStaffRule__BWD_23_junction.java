...
public class DocToStaffRule__BWD_23_junction extends GenericJunctionActor{
	private Map<Object, Collection<HMatch>> staff_5_AttrMap = HiPEMultiUtil.createMap();
	private Map<Object, Collection<HMatch>> department_5_AttrMap = HiPEMultiUtil.createMap();
	...
	protected void registerMatchForAttributeChanges(HMatch match) {
		...
		Collection<HMatch> staff_4_Matches = staff_5_AttrMap.get(matchObjects[4]);
		if(staff_4_Matches == null) {
			staff_4_Matches = new LinkedList<>();
			staff_5_AttrMap.put(matchObjects[4], staff_4_Matches);
		}
		
		staff_4_Matches.add(match);
		
		Collection<HMatch> department_0_Matches = department_5_AttrMap.get(matchObjects[0]);
		if(department_0_Matches == null) {
			department_0_Matches = new LinkedList<>();
			department_5_AttrMap.put(matchObjects[0], department_0_Matches);
		}
	...
	protected void deregisterMatchForAttributeChanges(Set<HMatch> matches, HMatch match) {
		Object[] matchObjects = match.getNodes();
		Collection<HMatch> matches_0 = staff_5_AttrMap.get(matchObjects[4]);
		if(matches_0 != null) {
			matches.remove(match);
		}
		Collection<HMatch> matches_1 = department_5_AttrMap.get(matchObjects[0]);
		if(matches_1 != null) {
			matches.remove(match);
		}
	}
	...
	protected void changeAttribute(AttributeChanged<HMatch> message) {
		...
		if(obj instanceof AdministrationExample.Staff) {
			if(staff_5_AttrMap.containsKey(obj)) {
				for(HMatch attr_match : staff_5_AttrMap.get(obj)) {
		...
		if(obj instanceof HospitalExample.Department) {
			if(department_5_AttrMap.containsKey(obj)) {
				for(HMatch attr_match : department_5_AttrMap.get(obj)) {
					for(int i=0; i<ports.size(); i++) {
						Port<HMatch> port = ports.get(i);
						HMatch match = attr_match;
