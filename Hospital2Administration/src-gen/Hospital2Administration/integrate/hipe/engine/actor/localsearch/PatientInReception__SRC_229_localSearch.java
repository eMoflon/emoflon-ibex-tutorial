...
public class PatientInReception__SRC_229_localSearch extends GenericLocalSearchActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor) {
		...
		ports.add(new PortJunction(node.getPorts().getPort().get(0), getSelf(), name2actor.get("PatientInReception__FWD_233_localSearch"), this::returnTrue  , false ));
		ports.add(new PortJunction(node.getPorts().getPort().get(1), getSelf(), name2actor.get("PatientInReception__SRC_production"), this::returnTrue  , false ));
	}
			...
			case "Hospital_object_SP0": 
				{
					...
					// h
					HMatch match_0 = new LocalSearchMatch("PatientInReception__SRC_229_localSearch", 3);
			...
			case "Patient_object_SP3": 
				{
					...
					// p1
					HMatch match_1 = new LocalSearchMatch("PatientInReception__SRC_229_localSearch", 3);
			...
			case "Reception_object_SP1": 
				{
					...
					// r
					HMatch match_2 = new LocalSearchMatch("PatientInReception__SRC_229_localSearch", 3);
		...
		switch(match.creator) {
			...
			case "PatientInReception_p1_lies_incoming_SRC__FILTER_NAC_SRC_219_localSearch": 
				if(nac_checker.deregister(match)) {
					HMatch acMatch_0 = new LocalSearchMatch("PatientInReception__SRC_229_localSearch", 3);
					Object[] acMatch_nodes_0 = acMatch_0.getNodes();
	...
	protected void addReference(ReferenceAdded msg) {
		...
		case "Hospital_reception_Reception": 
			{
				HMatch match = new LocalSearchMatch("PatientInReception__SRC_229_localSearch", 3);
				Object[] objs = match.getNodes();
				objs[0] = msg.source;
				objs[2] = msg.target;
				currentDepth++;
