...
public class RoomRule__SRC_272_localSearch extends GenericLocalSearchActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor) {
		...
		ports.add(new PortJunction(node.getPorts().getPort().get(0), getSelf(), name2actor.get("RoomRule__FWD_275_localSearch"), this::returnTrue  , false ));
		ports.add(new PortJunction(node.getPorts().getPort().get(1), getSelf(), name2actor.get("RoomRule__SRC_production"), this::returnTrue , 0  , false ));
	}
			...
			case "Department_object_SP2": 
				{
					...
					// d
					HMatch match_0 = new LocalSearchMatch("RoomRule__SRC_272_localSearch", 2);
			...
			case "Room_object_SP0": 
				{
					...
					// ro
					HMatch match_1 = new LocalSearchMatch("RoomRule__SRC_272_localSearch", 2);
	...
	protected void addReference(ReferenceAdded msg) {
		...
		case "Department_rooms_Room": 
			{
				HMatch match = new LocalSearchMatch("RoomRule__SRC_272_localSearch", 2);
				Object[] objs = match.getNodes();
				objs[0] = msg.source;
				objs[1] = msg.target;
				currentDepth++;
