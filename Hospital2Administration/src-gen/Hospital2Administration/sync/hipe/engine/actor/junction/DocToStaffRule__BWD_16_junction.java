...
public class DocToStaffRule__BWD_16_junction extends GenericJunctionActor{
	private Map<Object, Collection<HMatch>> department_39_AttrMap = HiPEMultiUtil.createMap();
	private Map<Object, Collection<HMatch>> staff_39_AttrMap = HiPEMultiUtil.createMap();
	...
	protected void registerMatchForAttributeChanges(HMatch match) {
		...
		Collection<HMatch> department_0_Matches = department_39_AttrMap.get(matchObjects[0]);
		if(department_0_Matches == null) {
			department_0_Matches = new LinkedList<>();
			department_39_AttrMap.put(matchObjects[0], department_0_Matches);
		}
		
		department_0_Matches.add(match);
		
		Collection<HMatch> staff_6_Matches = staff_39_AttrMap.get(matchObjects[6]);
		if(staff_6_Matches == null) {
			staff_6_Matches = new LinkedList<>();
			staff_39_AttrMap.put(matchObjects[6], staff_6_Matches);
		}
	...
	protected void deregisterMatchForAttributeChanges(Set<HMatch> matches, HMatch match) {
		Object[] matchObjects = match.getNodes();
		Collection<HMatch> matches_0 = department_39_AttrMap.get(matchObjects[0]);
		if(matches_0 != null) {
			matches.remove(match);
		}
		Collection<HMatch> matches_1 = staff_39_AttrMap.get(matchObjects[6]);
		if(matches_1 != null) {
			matches.remove(match);
		}
	}
	...
	protected void changeAttribute(AttributeChanged<HMatch> message) {
		...
		if(obj instanceof HospitalExample.Department) {
			if(department_39_AttrMap.containsKey(obj)) {
				for(HMatch attr_match : department_39_AttrMap.get(obj)) {
		...
		if(obj instanceof AdministrationExample.Staff) {
			if(staff_39_AttrMap.containsKey(obj)) {
				for(HMatch attr_match : staff_39_AttrMap.get(obj)) {
					for(int i=0; i<ports.size(); i++) {
						Port<HMatch> port = ports.get(i);
						HMatch match = attr_match;
